...
class GRU4Rec:
    ...
    def cross_entropy(self, yhat):
        return tf.reduce_mean(-tf.compat.v1.log(tf.compat.v1.diag_part(yhat)+1e-24))
    def bpr(self, yhat):
        yhatT = tf.transpose(yhat)
        return tf.reduce_mean(-tf.compat.v1.log(tf.nn.sigmoid(tf.compat.v1.diag_part(yhat)-yhatT)))
    def top1(self, yhat):
        yhatT = tf.transpose(yhat)
        term1 = tf.reduce_mean(tf.nn.sigmoid(-tf.compat.v1.diag_part(yhat)+yhatT)+tf.nn.sigmoid(yhatT**2), axis=0)
        term2 = tf.nn.sigmoid(tf.compat.v1.diag_part(yhat)**2) / self.batch_size
        return tf.reduce_mean(term1 - term2)

    def build_model(self):
        
        self.X = tf.compat.v1.placeholder(tf.int32, [self.batch_size], name='input')
        self.Y = tf.compat.v1.placeholder(tf.int32, [self.batch_size], name='output')
        self.state = [tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.rnn_size], name='rnn_state') for _ in range(self.layers)]
        ...
        with tf.compat.v1.variable_scope('gru_layer'):
            ...
            embedding = tf.compat.v1.get_variable('embedding', [self.n_items, self.rnn_size], initializer=initializer)
            softmax_W = tf.compat.v1.get_variable('softmax_w', [self.n_items, self.rnn_size], initializer=initializer)
            softmax_b = tf.compat.v1.get_variable('softmax_b', [self.n_items], initializer=tf.constant_initializer(0.0))
        ...
        self.lr = tf.maximum(1e-5,tf.compat.v1.train.exponential_decay(self.learning_rate, self.global_step, self.decay_steps, self.decay, staircase=True)) 
